...
class Essbase_status():
	...
	if stat_level!=0: #Indicates error encountered
		...
		if row==None:break
		if '%s' % int(row[1])!='0': #The session ID of 0 (Zero) is not included. This represents the script that made the connection
		    if row[0] not in listUser:
			listUser.append(row[0])
			dictUserSessions[row[0]]=['%s' % int(row[1])]
		    else:
			tempList=dictUserSessions[row[0]]
			tempList.append('%s' % int(row[1]))
			dictUserSessions[row[0]]=tempList
		    dictSessions['%s' % int(row[1])]=row
		    #print row
		    listTemp=[]
...
def main():
    ...
    for session in list(dictSession.keys()):
    ...
    strFull=""
    strLine=""
    for User in list(dictUserSession.keys()):
...
if __name__=='__main__':
    ...
    for Srvr in list(dict_use.keys()): #Creates netaddconnections based development or production servers
